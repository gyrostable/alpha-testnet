...
import { getBPoolsAddresses, getDeploymentConfig, scale } from "../misc/deployment-utils";
import {
  BPool,
  BPool__factory,
...
async function main() {
  ...
  const symbolMapping: Record<string, string> = { WETH: "ETH", BUSD: "DAI", sUSD: "DAI" };
  ...
  const getTokenInfo = async (pool: BPool, tokenAddress: string) => {
    ...
    return {
      ...
      weight: await pool.getNormalizedWeight(tokenAddress),
      decimals,
  ...
  for (const poolAddress of poolAddresses) {
    console.log("balancing pool ", poolAddress);
    ...
    const maxDeviation = scale(3, 16); // 3%
    ...
    if (deviationFirst.lte(maxDeviation)) {
      console.log("deviation too low, skipping");
      continue;
    }
