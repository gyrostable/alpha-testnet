...
describe("GyroFund", function () {
  ...
  const underlyingTokenSymbols = ["WETH", "DAI", "USDC"].map((symbol) =>
    utils.formatBytes32String(symbol)
  );
  ...
  beforeEach(async function () {
    ...
    mockPools = await Promise.all([mockPool1, mockPool2]);
  ...
  describe("check stablecoin health", function () {
    it("should check that only stablecoins near the peg are accepted", async function () {
      const stablecoinhealth = await gyroFund.checkStablecoinHealth(
        13,
